...
window.addReward = async function() {
  const inputEl = document.getElementById("rewardName");
  const costEl = document.getElementById("rewardCost");

  const rewardLabel = inputEl?.value.trim();
  const rewardPoints = parseInt(costEl?.value.trim(), 10);

  if (!rewardLabel || isNaN(rewardPoints)) {
    alert("Please enter valid reward details.");
    return;
  }

  const rewards = await getRewards();
  rewards.push({ name: rewardLabel, cost: rewardPoints });
...
window.redeemRewardChild = async function(childName, rewardName, rewardCost) {
  const stats = await getUserStats();
  if ((stats[childName]?.points || 0) < rewardCost) {
    alert("Not enough points.");
    return;
  }
  stats[childName].points -= rewardCost;
  await saveUserStats(stats);
