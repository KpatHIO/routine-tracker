...
window.addReward = async function addRewardHandler() {
  const inputNameEl = document.getElementById("rewardName");
  const inputCostEl = document.getElementById("rewardCost");

  const rewardNameValue = inputNameEl?.value.trim();
  const rewardCostValue = parseInt(inputCostEl?.value.trim(), 10);

  if (!rewardNameValue || isNaN(rewardCostValue)) {
    alert("Please enter valid reward details.");
    return;
  }

  const rewards = await getRewards();
  rewards.push({ name: rewardNameValue, cost: rewardCostValue });
  ...
  const rewards = await getRewards();
  rewards.push({ name: rewardName, cost });
  await saveRewards(rewards);
...
window.redeemReward = async function(index) {
  ...
  await saveUserStats(stats);
  alert(`${childName} redeemed "${reward.name}"!`);
  loadParentDashboard();
};
...
window.redeemRewardChild = async function(child, rewardName, cost) {
  ...
  stats[childName].points -= cost;
  await saveUserStats(stats);
  alert(`${childName} redeemed "${rewardName}"!`);
  loadChildView(childName);
};
